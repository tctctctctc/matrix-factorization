...
from MF.utils import semilogy
from MF.utils import get_k_fold_data
...
def main():
    # 加载数据
    data = pd.read_csv('data/movielens/ratings.dat', header=None, delimiter='::', engine='python',
                       names=['uid', 'iid', 'score'], usecols=[0, 1, 2])
    ...
    num_users, num_items = max(data.iloc[:, 0]) + 1, max(data.iloc[:, 1]) + 1
